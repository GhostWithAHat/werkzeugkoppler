from werkzeugkoppler.stream_chunks import (
    chunk_with_content_delta,
    content_to_thinking_value,
    passthrough_non_content_delta_chunk,
    reasoning_chunk,
    split_content_for_stream,
)
...
def test_passthrough_non_content_delta_keeps_unknown_fields_and_drops_content_and_tool_calls() -> None:
    ...
    out = passthrough_non_content_delta_chunk(
        upstream_chunk,
...
def test_split_content_for_stream_separates_reasoning_blocks_from_final_blocks() -> None:
    ...
    reasoning, final = split_content_for_stream(content)
...
def test_content_to_thinking_value_from_string_and_blocks() -> None:
    assert content_to_thinking_value("abc") == "abc"
    assert content_to_thinking_value([{"type": "output_text", "text": "a"}, {"text": "b"}]) == "ab"
